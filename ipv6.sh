...
IFS=$'\n\t'        # Internal Field Separator'ı güvenli hale getir

# IPv6 ağ ayarları
IPV6_SUBNET="2a00:5740:c:0::/64"
...
function get_subnet_mask(){
  if [ -z $subnet_mask ]; then
    ...
    # Önce mevcut IPv6 adreslerini kontrol et
    local existing_ipv6=$(ip -6 addr show dev "$interface_name" scope global | awk '/inet6/ {print $2}' | head -1)
    
    if [ -n "$existing_ipv6" ]; then
      # Mevcut IPv6 adresini kullan
      full_blocks_count=$(($subnet / 16));
      ipv6=$(echo "$existing_ipv6" | cut -d '/' -f1);
    else
      # Varsayılan IPv6 subnet'ini kullan
      ipv6="$IPV6_SUBNET"
    fi;
...
function get_backconnect_ipv4(){
  ...
  if [ ! -z "$backconnect_ipv4" ] && [ "$backconnect_ipv4" != " " ]; then echo "$backconnect_ipv4"; return; fi;
...
function configure_ipv6_network(){
  ...
  # IPv6 forwarding'ı etkinleştir
  echo "net.ipv6.conf.all.forwarding=1" >> /etc/sysctl.conf
  echo "net.ipv6.conf.default.forwarding=1" >> /etc/sysctl.conf
  echo "net.ipv6.conf.$interface_name.forwarding=1" >> /etc/sysctl.conf
  
  # IPv6 proxy NDP'yi etkinleştir
  ...
  # IPv6 non-local binding'i etkinleştir
  ...
  # IPv6 route ekle
  ip -6 route add "$IPV6_SUBNET" dev "$interface_name" &>> "$script_log_file" || true
  
  # IPv6 gateway route ekle
  ip -6 route add default via "$IPV6_GATEWAY" dev "$interface_name" &>> "$script_log_file" || true
  
  echo "IPv6 network configuration completed successfully"
}

function configure_ipv6_interface(){
  echo "Configuring IPv6 interface settings..."
  
  # Interface'e IPv6 adresi ekle
  ip -6 addr add "$IPV6_SUBNET" dev "$interface_name" &>> "$script_log_file" || true
  
  # Interface'i etkinleştir
  ip link set "$interface_name" up
  
  # IPv6 route'ları kontrol et
  if ip -6 route show | grep -q "$IPV6_SUBNET"; then
    echo "IPv6 subnet route configured successfully"
  else
    log_err "Warning: IPv6 subnet route configuration may have failed"
  fi
  
  if ip -6 route show | grep -q "default.*$IPV6_GATEWAY"; then
    echo "IPv6 default gateway configured successfully"
  else
    log_err "Warning: IPv6 default gateway configuration may have failed"
  fi
  
  echo "IPv6 interface configuration completed"
}
...
EOF
  ...
  # ndppd servisini yeniden başlat
  systemctl restart ndppd || service ndppd restart;

  # ndppd'nin çalışıp çalışmadığını kontrol et
  if systemctl is-active --quiet ndppd || pgrep -x ndppd > /dev/null; then
    echo "ndppd is up and running";
  else
    log_err "Warning: ndppd service may not be running properly";
  fi;
}
...
# IPv6 ağ ayarlarını kontrol et ve gerekirse yapılandır
if ! ip -6 route show | grep -q "$IPV6_SUBNET"; then
  echo "IPv6 network configuration not found, configuring..."
  configure_ipv6_network;
  configure_ipv6_interface;
fi;
...
echo -e "\n✅ IPv6 Proxy Server kurulumu tamamlandı!"
echo "🌐 IPv6 Subnet: $IPV6_SUBNET"
echo "🚪 Gateway: $IPV6_GATEWAY"
echo "🔧 Interface: $interface_name"
echo "📋 Proxy listesi: $backconnect_proxies_file"
